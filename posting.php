...
if ((	!$user->data['is_registered'] ||
	$user->data['user_posts'] < 50) && $mode == 'post') {
	$preview	= true; ## always preview for inexperienced users
}else{
...
if ($submit || $preview || $refresh)
{
	...
	*/
	...
	## an attempt to make links safe
	if (!function_exists('FilterMessage')){include_once $phpbb_root_path . '/spamFilter.php';;}
	$message_parser->message = FilterMessage($message_parser->message, $user, $post_data);
	##
	...
	if (!count($error) && $submit)
	{
		if ($submit)
		{
			...
			*/
			...
			if ($guest_spam || (!$auth->acl_get('f_noapprove', $data['forum_id']) && empty($data['force_approved_state'])) || (isset($data['force_approved_state']) && !$data['force_approved_state']))
			{
